...
当前不能直接上架。必须关闭全部 P0，并完成 Release 包真机回归与 Play Console 预检查。

当前 P0 分两类：

| 分类 | 条目 | 下一步 |
| --- | --- | --- |
| 真正待执行 P0 | P0-2、P0-5、P0-7、P0-8 | 必须继续补材料、代码或发布验证 |
| 已修复待验证 P0 | P0-1、P0-3、P0-4、P0-6 | 不优先写新代码，先补测试、Release、真机验收记录 |

说明：P0-3、P0-4、P0-6 在状态机、动画和边界场景上有交叉，执行验收时可以合并跑同一套真机回归，避免重复劳动。
...
**待验证**：真机或测试环境中模拟读取失败，确认错误态可重试；恢复授权或恢复读取能力后可重新加载。
...
**已完成**：仓库内已维护 `docs/PRIVACY_POLICY.md`。

**未完成**：隐私政策尚未确认已发布到公网 URL；Play Console 尚未确认填写同一 URL；Data safety 表单尚未确认与实际行为一致。

**方案**：发布隐私政策到公网 URL；Play Console 填写同一 URL；Data safety 表单如实声明是否收集、上传、共享照片，是否使用第三方 SDK、广告、分析或云端处理。
...
**待验证**：真机确认删除、恢复、取消删除、确认删除后状态与 UI 一致。
...
**待验证**：真机快速乱划、连续上划、快速上划后下划、左右连续滑动。
...
**已完成**：Manifest 声明 Android 10-12、Android 13+、Android 14+ 部分授权权限；UI 有基础授权请求、rationale 和部分授权提示。

**未完成**：拒绝后进入设置的闭环不明确；从设置返回后重新检查权限并重新加载不明确；部分授权变化后的刷新路径未真机验证；权限撤销后重进稳定性未验证。
...
**待验证**：真机确认空相册、单张照片、最后一张删除、全部待删确认页。
...
**当前风险**：`deleteDirectly()` 仍返回 Boolean；API 29 没有结构化处理 `RecoverableSecurityException`；批量删除不能表达全部成功、部分成功、失败、需要授权；失败时 UI 无结构化反馈。
...
1. P0-7 删除行为安全。
2. P0-5 权限完整流程。
3. P0-2 隐私政策公网 URL 和 Play Console 填写。
