...
| 分类 | 条目 | 下一步 |
| --- | --- | --- |
| 真正待执行 P0 | P0-2、P0-8 | 必须继续补发布材料或发布验证 |
| 已修复待验证 P0 | P0-1、P0-3、P0-4、P0-5、P0-6、P0-7 | 不优先写新代码，先补测试、Release、真机验收记录 |
...
**待验证**：Fresh install 权限弹窗；拒绝后页面不白屏、不 Loading；永久拒绝后打开系统设置并授权；从设置返回后重新加载；Android 14+ 部分授权只展示授权照片；权限撤销后重进不崩溃。
...
1. P0-2 隐私政策公网 URL 和 Play Console 填写。
2. P0-8 Release 包构建与真机回归。
