...
| 分类 | 条目 | 下一步 |
| --- | --- | --- |
| 真正待执行 P0 | P0-2、P0-5、P0-8 | 必须继续补材料、代码或发布验证 |
| 已修复待验证 P0 | P0-1、P0-3、P0-4、P0-6、P0-7 | 不优先写新代码，先补测试、Release、真机验收记录 |

说明：P0-3、P0-4、P0-6、P0-7 在状态机、动画、边界和删除结果上有交叉，执行验收时可以合并跑同一套真机回归，避免重复劳动。
...
**待验证**：API 30+ 真机系统弹窗；API 29 真机 `RecoverableSecurityException` 授权路径；API 28 及以下直接删除；批量删除中部分失败；用户取消系统弹窗后队列保留。API 29 多张照片可能触发多次授权，需要真机确认是否能通过重复重试完成全部删除。
